"""HTTP routes for referrals-service.

Generic CRUD over the `referrals` table. Every mutating endpoint publishes a
...
def build_blueprint(svc) -> Blueprint:
    ...
    def create_record():
        ...
        if payload.get('referred_to_id'):
            _peer = clients.get('providers-service')
            if _peer:
                try:
                    payload['referred_to_snapshot'] = json_or_raise(
                        _peer.get(f"/api/providers/" + str(payload['referred_to_id']))
                    )
                except ServiceUnavailable as _e:
                    payload['referred_to_warn'] = str(_e)
        row = db.query_one(
